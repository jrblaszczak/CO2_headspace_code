...
notes <- read.table("2018_08_31_BeaverDiel_notes.txt", sep=",",header=T)

notes <- notes %>% 
  separate(SampleID, c("Site","Date","Time"))

notes$Date <- revalue(notes$Date, c("830" = "2018-08-30","831"="2018-08-31"))
...
#######################################################
iso <- read.csv("2018_08_31_BeaverDiel_Picarro.csv", header=T)
...
iso$Date <- revalue(iso$Date, c("830" = "2018-08-30","831"="2018-08-31"))
...
## Export
#write.csv(iso_mean_sd,"2018_08_31_Beaver_iso_CO2conc.csv")
...
## pH Sensor ##
###############
pHraw <- read.csv("pH_logfile_2018_09_04_BeaverCreek.csv", header=T)
...
pH_10min$DateTime <- lubridate::round_date(pH_10min$DateTime, "5 minutes") 

ggplot(pH_10min, aes(DateTime, pH))+
  geom_point()+
  scale_y_continuous(limits = c(7.75, 8.75))
